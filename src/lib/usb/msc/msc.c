...
void MSC_MemoryRead(void)
{
  ...
  USB_EpWrite(USB_MSC_EP_BLK_I, &gMemory[gRwOffset], n);
  gRwOffset += n;
  gRwLength -= n;
...
void MSC_SetCSW(void)
{
  ...
  USB_EpWrite(USB_MSC_EP_BLK_I, (U8 *)&gCSW, sizeof(gCSW));
...
void MSC_MemoryWrite(void)
{
  ...
  if ((gRwOffset + gBulkRwLength) > MSC_MemorySize)
  {
    ...
    gBulkStage = MSC_BS_CSW;
    USB_EpSetStall(USB_MSC_EP_BLK_O);
  }
...
U32 MSC_RWSetup(void)
{
  ...
  {
    USB_EpSetStall(USB_MSC_EP_BLK_I);
    USB_EpSetStall(USB_MSC_EP_BLK_O);
...
void DataInTransfer(void)
{
  ...
  USB_EpWrite(USB_MSC_EP_BLK_I, gBulkBuf, gBulkRwLength);
  gBulkStage = MSC_BS_DATA_IN_LAST;
...
{
  gBulkRwLength = USB_EpRead(USB_MSC_EP_BLK_O, gBulkBuf, USB_MSC_PACKET_SIZE);
  switch (gBulkStage)
  {
    case MSC_BS_CBW:
      MSC_GetCBW();
...
void MSC_Init(void)
{
  ...
  USB_SetCb_Ep(USB_MSC_EP_BLK_I, msc_BulkIn);
  USB_SetCb_Ep(USB_MSC_EP_BLK_O, msc_BulkOut);
}
